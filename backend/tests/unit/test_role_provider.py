...
def test_scan_yields_one_asset_per_prose_file(role_tree: Path) -> None:
...
def test_role_json_is_never_an_asset(role_tree: Path) -> None:
    provider = MasterworkRoleProvider(store_root=role_tree)
    assert all(a.path.name != "role.json" for a in provider.scan())
    assert provider.asset_id_for_path(role_tree / "plan" / "role.json") is None
...
def test_weird_role_names_are_skipped_not_mangled(role_tree: Path) -> None:
    ...
    assert {a.name for a in provider.scan()} == {
        "plan:system",
        "plan:user",
        "build:system",
        "build:user",
...
def test_stray_files_and_nested_dirs_are_not_assets(role_tree: Path) -> None:
    ...
    provider = MasterworkRoleProvider(store_root=role_tree)
    assert len(list(provider.scan())) == 4
...
def test_asset_id_round_trips_through_parse_asset_id(role_tree: Path) -> None:
    provider = MasterworkRoleProvider(store_root=role_tree)
    for role, part in (("plan", "system"), ("build", "user")):
        path = role_tree / role / f"{part}.md"
        asset_id = provider.asset_id_for_path(path)
