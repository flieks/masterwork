...
async def test_project_second_message_resumes_with_state_line(
    ...
    # Later message resumes, re-sends the system prompt and prepends the state line.
    assert runner.calls[1]["resume"] == "cli-p4"
    assert "This chat is scoped to a PROJECT" in (runner.calls[1]["system_prompt"] or "")
