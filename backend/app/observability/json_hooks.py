"""The shared shape of a JSON hooks file, and an integration that edits one.

Claude Code's `settings.json` and Codex's `hooks.json` both hold their hooks as
`{"hooks": {"<Event>": [{"matcher": …, "hooks": [{"type": "command", …}]}]}}`,
so everything about finding our entries, adding them and removing them again is
one implementation. An agent is a subclass that names its events, its matchers
and the marker its entries are recognised by; wording follows `label`.
...
def _is_ours(hook: object, markers: tuple[str, ...]) -> bool:
    return (
...
def _ours_in(
    hooks: dict[str, Any], event: str, markers: tuple[str, ...]
    ...
    for group in _groups(hooks, event):
        ...
        if not isinstance(entries, list):
            continue
        found.extend((group.get("matcher"), e) for e in entries if _is_ours(e, markers))
    return found
...
def _without_ours(
    ...
    for group in _groups(hooks, event):
        ...
        if not isinstance(entries, list):
            ...
            continue
        remaining = [entry for entry in entries if not _is_ours(entry, markers)]
        if remaining:
...
class JsonHooksIntegration:
    """Wires one agent's JSON hooks file to the ingest endpoint."""

    id: str
    label: str
    # The events subscribed once connected, and the matcher each one gets.
    events: ClassVar[list[str]]
    matchers: ClassVar[dict[str, str | None]] = {}
    # How an entry of ours is recognised in a file we did not write in this process.
    markers: ClassVar[tuple[str, ...]] = ("masterwork",)
    # Hooks that must finish before the agent moves on — the process may exit
    ...
    def _assess(self, settings: dict[str, Any]) -> tuple[IntegrationState, str]:
        ...
        wired = {event: _ours_in(hooks, event, self.markers) for event in self.events}
        if not any(wired.values()):
            return (
                "disconnected",
                f"{self.label} isn't reporting its sessions yet. Connecting adds "
                f"{len(self.events)} hooks to {self._settings_path} — nothing else on your "
                "machine changes.",
            )

        expected = self._expected()
        stale = [event for event in self.events if wired[event] != [expected[event]]]
        if stale:
            # Naming every one of them reads as noise; a few is the useful case.
            which = "" if len(stale) == len(self.events) else f" ({', '.join(stale)})"
            return (
                "outdated",
        ...
        return "connected", f"Recording every {self.label} session to {self._ingest_url}."
    ...
    def connect(self) -> IntegrationStatus:
        ...
        for event in self.events:
            ...
            hooks[event] = [*_without_ours(hooks, event, self.markers), group]
        after["hooks"] = hooks
    ...
    def disconnect(self) -> IntegrationStatus:
        ...
        for event in list(hooks):
            kept = _without_ours(hooks, event, self.markers)
    ...
    def _blocker(self) -> str | None:
        ...
        if not self._settings_path.parent.exists():
            return (
                f"{self.label} hasn't run on this machine yet — {self._settings_path.parent} "
    ...
    def _hook_entry(self, event: str, command: str) -> dict[str, Any]:
        entry: dict[str, Any] = {
            "type": "command",
            "command": command,
    ...
    def _expected(self) -> dict[str, tuple[str | None, dict[str, Any]]]:
        ...
        return {
            event: (self.matchers.get(event), self._hook_entry(event, command))
            for event in self.events
        }
