...
test.describe('global instructions', () => {
  test.skip(!process.env.BACKEND_URL, 'requires a running backend (set BACKEND_URL)');

  test('opens from the sidebar, switches agents and toggles the editor', async ({ page }) => {
    await page.goto('/skills');
    await page.getByRole('link', { name: 'Instructions' }).click();

    // The default tab follows the active assistant, so pick one explicitly.
    ...
    await page.getByRole('tab', { name: 'Claude Code · CLAUDE.md' }).click();
